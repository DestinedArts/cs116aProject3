...
public: 
	...
	//
	// for drawing a shape from parent to child that stays with it when transforming
	// additional work to override
	// do not create another scene for the object
	...
	~SceneObject()
	{
		// if there is a parent, tell parent to remove this object from its child list
		if (parent != nullptr)
			parent->removeChild(this);
		else // tell all of its children they have no parent anymore
			for (SceneObject* child : childList)
			{
				child->parent = nullptr;
				child->position += position; // adjust child position so it does not change after its parent is deleted
			}
	}

	void addChild(SceneObject *child) {
		cout << "Add " << child->name << " to " << name << endl;
	...
	void removeChild(SceneObject* child)
	{
		...
		{
			// find the child to remove from the child list
			if (childList[i] == child)
			{
				// remove child from this child's list of children
				child->parent = nullptr;
				childList.erase(childList.begin() + i);
				// add child's children to this child's list of children (i.e. grandchildren become children)
				for (int j = 0; j < child->childList.size(); j++)
				{
					// adjust position so it does not change after deleting its parent
					child->childList[j]->position += child->position;
					addChild(child->childList[j]);
				}
			}
		}
	}
...
{
public:
	Joint(string name, glm::vec3 p, float r, ofColor diffuse = ofColor::lightGray) : Sphere(p, r, diffuse) 
	{
		this->name = name;
	}
